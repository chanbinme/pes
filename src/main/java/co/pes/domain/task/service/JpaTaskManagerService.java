...
public class JpaTaskManagerService extends AbstractTaskManagerService {
    ...
    private final JpaOrganizationRepository organization;
    ...
    public List<Tasks> getTasks(String year, String projectTitle) {
        ...
        for (Tasks task : taskList) {
            ...
            if (!CollectionUtils.isEmpty(chargeTeamIds)) {
                ...
                List<String> chargeTeamTitles = organization.searchChargeTeamTitlesByTeamIds(chargeTeamIds);
    ...
    protected void resetMapping(Mapping mappingInfo) {
        List<TaskOrganizationMappingEntity> taskOrganizationMappingEntityList = taskOrganizationMappingRepository.findAllByTaskId(mappingInfo.getTaskId());
        if (!CollectionUtils.isEmpty(taskOrganizationMappingEntityList)) {
            for (TaskOrganizationMappingEntity taskOrganizationMappingEntity : taskOrganizationMappingEntityList) {
                if (totalService.existsTotal(taskOrganizationMappingEntity.getOrganization().getId())) {
                    log.info("resetMapping exception occur mapping : {}", findMappingInfo.toString());
                    throw new BusinessLogicException(ExceptionCode.INVALID_MAPPING);
                }
            }
